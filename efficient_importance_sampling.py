...
class RegionOptimizer:
    ...
    def solve_kkt_system(self, A_indices: List[int], constraints: Dict) -> Tuple[np.ndarray, float]:
        """
        ...
        def objective_and_constraints(theta):
            ...
            if 'siegmund' in constraints:
                ...
                obj = sum(u * theta[k] for k in A_indices) - sum(ell * theta[k] for k in range(self.d) if k not in A_indices)
                ...
                obj = min(sum(sorted_theta[L-ell:L+ell]) / (2*ell+1) for ell in range(1, L+1))
            else:
        ...
        try:
            result = opt.minimize(lagrangian, x0, method='SLSQP', 
    ...
    def _fallback_optimization(self, A_indices: List[int], constraints: Dict) -> Tuple[np.ndarray, float]:
        ...
        for _ in range(100):
            ...
            if abs(lambda_val) > 1e-8:
                ...
                try:
                    theta -= lambda_val * np.linalg.solve(hess, grad) / np.dot(grad, np.linalg.solve(hess, grad))
                except:
                    theta -= 0.01 * grad
        ...
        if 'siegmund' in constraints:
            u, ell = constraints['siegmund']['u'], constraints['siegmund']['ell']
            rate = sum(u * theta[k] for k in A_indices) - sum(ell * theta[k] for k in range(self.d) if k not in A_indices)
        else:
...
class MultidimensionalSiegmund:
    ...
    def __init__(self, d: int, ell: float, u: float, mean: np.ndarray, covariance: np.ndarray):
        """
    ...
    def simulate_wrong_exit_probability(self, b: float, n_samples: int = 10000, 
                                      use_feasible_mixture: bool = True) -> SimulationResult:
        """
        ...
        if use_feasible_mixture:
            ...
            if self.d > 10:
                warnings.warn(f"Full mixture with d={self.d} has {2**self.d-1} components. This may be slow.")
        ...
        for sample_idx in range(n_samples):
            ...
            if wrong_exit:
                ...
                log_likelihood_ratio = -np.dot(theta, position) + n_steps * self.cgf.Lambda(theta)
...
class SumIntersectionRule:
    ...
    def compute_feasible_mixture(self) -> Tuple[List[np.ndarray], List[float]]:
        ...
        for subset in subsets_L_plus_1[:min(100, len(subsets_L_plus_1))]:  # Limit for efficiency
...
def run_comprehensive_example():
    ...
    for b in b_values:
        # Feasible mixture
        result_feasible = siegmund.simulate_wrong_exit_probability(b, n_samples=5000, use_feasible_mixture=True)
        
        print(f"b = {b}:")
        print(f"  Feasible mixture: P = {result_feasible.estimate:.2e} ± {result_feasible.std_error:.2e}")
        ...
        if d <= 6:
            result_full = siegmund.simulate_wrong_exit_probability(b, n_samples=1000, use_feasible_mixture=False)
            print(f"  Full mixture: P = {result_full.estimate:.2e} ± {result_full.std_error:.2e}")
        print()
    ...
    cov_gap = sigma_sq * (np.eye(d_gap) + rho_gap * (np.ones((d_gap, d_gap)) - np.eye(d_gap)))
...
if __name__ == "__main__":
    ...
    for d in dimensions:
        ...
        result = prob.simulate_wrong_exit_probability(2.0, n_samples=1000, use_feasible_mixture=True)
        times_feasible.append(result.computation_time)
        
        # Time full mixture (only for small d)
        if d <= 4:
            result_full = prob.simulate_wrong_exit_probability(2.0, n_samples=100, use_feasible_mixture=False)
    ...
    for i, d in enumerate(dimensions):
        if not np.isnan(times_full[i]):
            ...
            print(f"    {d}     |    {times_feasible[i]:.3f}s     |  {times_full[i]:.3f}s   |  {speedup:.1f}x")
        else:
